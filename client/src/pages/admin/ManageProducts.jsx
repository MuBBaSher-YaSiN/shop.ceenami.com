...
export default function ManageProducts() {
  ...
  <div className="w-full max-w-screen-xl bg-white/5 backdrop-blur-md text-white p-3 sm:p-4 md:p-6 rounded-xl border border-[#d5b56e] shadow-2xl">
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
      ...
      <button className="bg-[#d5b56e] text-black px-3 sm:px-4 py-1 sm:py-2 rounded hover:bg-yellow-600 transition font-semibold text-xs sm:text-sm">
        + Add Product
      </button>
    </div>

    {isLoading ? (
      <p className="text-white text-center text-sm sm:text-base">Loading products...</p>
    ) : isError ? (
      <p className="text-red-400 text-center text-sm sm:text-base">Failed to load products</p>
    ) : (
